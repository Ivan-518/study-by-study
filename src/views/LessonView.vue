...
const emit = defineEmits<{ back: []; 'update-progress': [id: string, progress: number]; add: [note: string] }>()
...
<template>
  <main v-if="lesson" class="page lesson-page">
    ...
    <div class="lesson-layout"><section class="panel lesson-content"><nav class="lesson-tabs"><button type="button" :class="{ active: activeLayer === 'overview' }" @click="activeLayer = 'overview'">1. 核心理解</button><button type="button" :class="{ active: activeLayer === 'code' }" @click="activeLayer = 'code'">2. 最小代码</button><button type="button" :class="{ active: activeLayer === 'practice' }" @click="activeLayer = 'practice'">3. 练习与延伸</button></nav>
        <article v-if="activeLayer === 'overview'" class="lesson-section"><h2>先建立正确的直觉</h2><p>{{ lesson.overview }}</p><h3>关键要点</h3><ol><li v-for="point in lesson.keyPoints" :key="point">{{ point }}</li></ol><section class="prerequisite-card"><i class="pi pi-sitemap" /><div><strong>需要的前置知识</strong><p>{{ lesson.prerequisites.join('、') }}。如果这里有不熟悉的概念，先在知识地图中补齐，再回到本节。</p></div></section><button type="button" class="primary-button" @click="activeLayer = 'code'; markProgress(Math.max(progress, 35))">查看最小代码 <i class="pi pi-arrow-right" /></button></article>
