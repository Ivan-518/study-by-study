...
<template>
  <main class="page library-page"><section class="page-heading grid-heading"><h1>知识库</h1><p>沉淀已学内容，随时回到你的上下文。</p></section><div class="global-search compact-search library-search"><i class="pi pi-search" /><input placeholder="搜索已保存的资料、笔记或概念..." /><button aria-label="筛选"><i class="pi pi-sliders-h" /></button></div>
    ...
    </div><section class="panel deep-dive"><header class="panel-title"><strong>待深入</strong><button class="link-button" type="button" @click="emit('navigate', 'path')">继续整理 <i class="pi pi-arrow-right" /></button></header><div><article v-for="title in ['GraphRAG：让知识图谱增强 RAG', 'ReAct 思维模式详解与实践', '评估 RAG：指标与方法']" :key="title"><div class="tech-thumb blocks"><i class="pi pi-circle-fill" /></div><h3>{{ title }}</h3><div class="tag-list"><span>RAG</span><span>知识图谱</span></div><small>学习于 2024-05-16　16 分钟阅读</small><i class="pi pi-bookmark" /><i class="pi pi-ellipsis-v" /></article></div></section>
  </main>
</template>
